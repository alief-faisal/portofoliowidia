...
function createTextTexture(
  gl: GL, text: string, font: string = 'bold 30px monospace', color: string = 'black'
...
class Media {
  extra: number = 0; geometry: Plane; gl: GL; image: string; index: number; length: number;
...
class AppGL {
  ...
  createMedias(items: any, bend: number, textColor: string, borderRadius: number, font: string) {
    const defaultItems = [
      { image: 'https://images.unsplash.com/photo-1506744038136-46273834b3fb?w=600', text: 'Landscape' },
      { image: 'https://images.unsplash.com/photo-1469474968028-56623f02e42e?w=600', text: 'Nature' },
      { image: 'https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=600', text: 'Beach' },
      { image: 'https://images.unsplash.com/photo-1519681393784-d120267933ba?w=600', text: 'Mountain' },
      { image: 'https://images.unsplash.com/photo-1476514525535-07fb3b4ae5f1?w=600', text: 'Lake' },
      { image: 'https://images.unsplash.com/photo-1472214103451-9374bd1c798e?w=600', text: 'Meadow' },
      { image: 'https://images.unsplash.com/photo-1501785888041-af3ef285b470?w=600', text: 'Valley' },
      { image: 'https://images.unsplash.com/photo-1470071459604-3b5ec3a7fe05?w=600', text: 'Forest' },
      { image: 'https://images.unsplash.com/photo-1441974231531-c6227db76b6e?w=600', text: 'Woodland' },
      { image: 'https://images.unsplash.com/photo-1518173946687-a1e23d925093?w=600', text: 'Sunset' },
    ];
  ...
  onWheel(e: Event) { const we = e as WheelEvent; const d = we.deltaY || 0; this.scroll.target += (d > 0 ? this.scrollSpeed : -this.scrollSpeed) * 0.2; this.onCheckDebounce(); }
...
interface CircularGalleryProps {
  ...
  bend?: number; textColor?: string; borderRadius?: number; font?: string;
  scrollSpeed?: number; scrollEase?: number;
}

export default function CircularGallery({ items, bend = 3, textColor = '#ffffff', borderRadius = 0.05, font = 'bold 30px Inter', scrollSpeed = 2, scrollEase = 0.05 }: CircularGalleryProps) {
  const containerRef = useRef<HTMLDivElement>(null);
  useEffect(() => {
    if (!containerRef.current) return;
    const app = new AppGL(containerRef.current, { items, bend, textColor, borderRadius, font, scrollSpeed, scrollEase });
    return () => { app.destroy(); };
